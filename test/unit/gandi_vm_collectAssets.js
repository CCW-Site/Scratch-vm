...
test('collectAssetsWithGandiAssets', t => {
    ...
    sprite.costumes = [{id: 3, asset: costumeAsset1}];

    vm.runtime.gandi = {assets: [{id: 4, asset: gandiAsset1}], wildExtensions: {}};

    const assets = vm.assets;
    t.type(assets.length, 'number');
    t.equal(assets.length, 4);
    t.same(assets, [soundAsset1, soundAsset2, costumeAsset1, gandiAsset1]);
    t.end();
});
